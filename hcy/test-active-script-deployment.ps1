...
try {
    ...
    Set-Content -LiteralPath (Join-Path $source 'executor.js') -Encoding utf8 -Value 'throw error; // terminal recovery v2'
    Set-Content -LiteralPath (Join-Path $alias 'manifest.json') -Encoding utf8 -Value '{"version":"1.0.2-hcy.3","saved_files":[]}'
    Set-Content -LiteralPath (Join-Path $alias 'executor.js') -Encoding utf8 -Value 'throw error; // terminal recovery v2'
    Set-Content -LiteralPath (Join-Path $active 'executor.js') -Encoding utf8 -Value 'throw error; // old code matches historical marker'
    ...
    Set-Content -LiteralPath (Join-Path $active 'executor.js') -Encoding utf8 -Value 'throw error; // terminal recovery v2'
