(async function () {
    ...
    async function enter_stage() {
        ...
        // 输入GUID，并轮换奇域
        selectedGuid = guidList[(guidList.length + switchCount) % guidList.length];
        inputText(`${selectedGuid}`);
        switchCount++;
            ...
            switch (current_ui) {
                ...
                case "奇域：游玩界面":
                    ...
                    if (settings.self_set) {
                        ...
                        if (!selectedGuid || typeof archiveList[selectedGuid] !== "string")
                            throw new Error(`没有本次已选择奇域的动作记录: ${selectedGuid}`);
                        let cmdList = archiveList[selectedGuid].split("|");
